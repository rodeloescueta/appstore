...
<!-- <br> -->
    <v-card-title>Tasks
        <v-spacer/>
            <v-combobox v-model="section" label="Section" :items="SectionOpt" @click:clear="section='' " clearable dense></v-combobox>
        ...
        fixed-header
        :search="searchTasks"
        :loading="Staffs.length>1? false : true"
        >
         <template v-slot:item.action="{item}">
                            <td>
                              <v-btn text icon @click="otherTask(item)">
        ...
        <template v-slot:item.tasks="{item}">{{item.tasks[0]}}</template>
        <template v-slot:item.percentage="{item}">
            <v-progress-linear
                    :value="item.percentage[0]"
                    :color="getColor(item.percentage[0])"
                    height="40"
                    v-if="item.percentage[0]!=undefined"
                  ><strong v-if="item.percentage[0]!=undefined" >Percentage: {{ Math.ceil(item.percentage[0])}}%</strong>
                  
                  
            </v-progress-linear>
        </template>
        <template v-slot:item.tdate="{item}">{{item.tdate[0]}}</template>
        <template v-slot:item.details="{item}">{{item.details[0]}}</template>
        <template v-slot:item.avatar="{item}">
                    <v-avatar size="36px"><img :src="avatarUrl(item.employee_code)"></v-avatar> 
         </template>
        </v-data-table>
    ...
    <!-- DialogLineUpToDos -->
        <v-dialog v-model="dialogOthers">
            <v-card>
                <v-toolbar dark dense>
                    <v-toolbar-title>Other Tasks</v-toolbar-title>
                </v-toolbar>
                <v-list v-for="(item,i) in otherTasks.tasks" :key="i">
                <v-list-item>
                <v-list-item-action>
                ...
                <v-list-item-content>
                <v-list-item-title >
                  {{item + ' ' + otherTasks.percentage[i] +' %'}}
                  </v-list-item-title>
                  <v-list-item-subtitle>
                      {{otherTasks.details[i]}}
                  </v-list-item-subtitle>
               </v-list-item-content>      
          </v-list-item> 
            </v-list>
...
export default {
    ...
    data(){
        return{
            ...
            rndNum:[],
            SectionOpt:['SOFTWARE DEVELOPMENT 1','SOFTWARE DEVELOPMENT 2','SOFTWARE DEVELOPMENT 3'],
            section:null,
            Staffs:[],
            headersTask:[
                        {text:'',value:'avatar'},
                        ...
                        {text: 'System Name', value:'tasks'},
                        {text:'Percentage %', value:'percentage', width:180},
...
// },
   methods:{
       ...
       otherTask(data){
            ...
            console.log(data)
            this.otherTasks = Object.assign({},data)
            // this.otherTasks.tasks.splice(0,1)
            // this.otherTasks.percentage.splice(0,1)
            // this.otherTasks.tdate.splice(0,1)
            // this.otherTasks.details.splice(0,1)
       ...
       generateTodos(){  
            axios.get(`${this.api}getSystems`).then(res=>{
                this.allTasks = res.data.filter(rec=>{
       ...
       assignedTodos(){
            for(let i=0; i<this.Staffs.length;i++){
                // let http = new XMLHttpRequest();
                //  http.open('HEAD', `http://hrdapps48:3001/ftp/employee_pictures/0${this.Staffs[i].employee_code}.jpg`, false);
                //  http.send()
                //  if(http.status != 404){
                //       this.Staffs[i].avatar = `http://hrdapps48:3001/ftp/employee_pictures/0${this.Staffs[i].employee_code}.jpg`
                //  }
                //  else {
                //      this.Staffs[i].avatar = `https://img.pngio.com/no-avatar-png-transparent-png-download-for-free-3856300-trzcacak-png-avatar-920_954.png`
                //  }

                for(let y=0;y<this.Staffs.length;y++){
                      this.Staffs[y].tasks = []
                      this.Staffs[y].percentage = []
                      this.Staffs[y].tdate = [] 
                      this.Staffs[y].details = [] 
                      
                for(let x=0;x<this.allTasks.length;x++){
                    let tasks = this.allTasks[x].incharge
                    for(let z=0;z<tasks.length;z++){
                    if(tasks[z].value==this.Staffs[y].employee_code && this.Staffs[y].section == this.allTasks[x].section){
                         this.Staffs[y].tasks[this.Staffs[y].tasks.length] = this.allTasks[x].sname
                         this.Staffs[y].percentage[this.Staffs[y].percentage.length] = this.allTasks[x].percentage
                         this.Staffs[y].tdate[this.Staffs[y].tdate.length] = this.allTasks[x].tdate
                         this.Staffs[y].details[this.Staffs[y].details.length] = this.allTasks[x].details      
                    }
                }
                }
                }
    ...
    //   },
       getColor (data) {
        // console.log(data)
        if (data != undefined &&  data!='0') return 'blue'
        else if(data == undefined) return 'green'
        else return 'red'
      }
   },
   computed:{
       StaffsFilter(){
           if(!this.section){
               ...
               let data = this.Staffs.filter(rec=>{
                   return rec.section == this.section
               })
               return data
           }
       },
