<template>
  <v-app class="myApp" id="inspire">
    <v-navigation-drawer :clipped="$vuetify.breakpoint.lgAndUp" app :mini-variant="drawer">
      <v-divider></v-divider>
        <v-list class="pt-0" dense>
          <template v-for="item in items">
            <template v-if="item.avatar">
              <v-list-item :key="item.text" @click="changePage(item.text)" :to="item.to">
                <v-list-item-avatar>
            ...
            <template v-else>
              ...
              <v-list-item v-else :key="item.text" @click="changePage(item.text)" :to="item.to">
                <v-list-item-action>
          ...
          <v-list-item v-if="userInfo.department=='QA'" :to="system.to">
           ...
           <v-list-item v-if="userInfo.department=='QA' && userInfo.designation !='Japanese Advisor'" :to="manage.to">
            <v-list-item-action>
          ...
          <v-list-item v-if="userInfo.department=='QA' && userInfo.designation !='Japanese Advisor'" :to="task.to">
            <v-list-item-action>
      ...
      <v-app-bar dense flat :clipped-left="$vuetify.breakpoint.lgAndUp" app>
        <v-toolbar-title style="color: blue;">
          ...
          <v-icon medium class="pb-2 pl-3" color="secondary" left @click.stop="drawer = !drawer">menu</v-icon>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            ...
            <v-avatar size="36">
              <img @error="onEmptyProfile"  v-on="on" icon  @click="updateMyProfile(userInfo.employeeCode)" :src="profile">
            </v-avatar>
      ...
      <template>
        <v-dialog v-model="dialogPersonalInfo" fullscreen persistent hide-overlay transition="dialog-bottom-transition">
          <v-card>
            <user-info-dialog/>
          </v-card>
        </v-dialog>
              ...
              <v-row dense>
                ...
                <v-col cols="12">
                  ...
                  >
                    ...
                    <div>
                      ...
                      Suggestion or Problem encounter with this version, message me @(i.p. @delo or in this messaging function).
                      We can still used the old version
                    </div>

                    <v-card-actions>
                      <v-btn block @click="openNewTab('http://hrdapps48:5050')">Open previous version</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-col>
                ...
                <v-col cols="12">
                  <v-card
                    ...
                    dark class="pa-2"
                  >
                    <v-card-title class="headline">1.) Messaging Function</v-card-title>

                    <div>
                      Alternative for internal usage of email account
                      <br>
                      Insert attachment but not image is still on going. (like: pdf, xls)
                    </div>

                    <v-card-actions>
                      <v-btn block @click="openNewTab('http://hrdapps30/system_team/doku.php?id=funciton_howtouse:messaging')">
                        Details How To Use
                      </v-btn>
                    </v-card-actions>
                  </v-card>
                ...
                <v-col cols="12">
                  <v-card
                    ...
                    dark class="pa-2"
                  >
                    <v-card-title class="headline">2.) Interface Standardization</v-card-title>

                    <div>
                      Web Application created by QA@hrd team will follow 1 Interface design.
                      <br>
                      Like PCMS, HRD System and J.A. System
                    </div>

                    <v-card-actions>
                      <v-btn block @click="openNewTab('http://hrdapps30/system_team/doku.php?id=interface_standardization')">Details</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-col>
...
// import axios from 'axios'
  ...
  export default {
    ...
    mounted() {
      ...
      // this.getRecentMessages() // ********************************get recent messages*************
      // this.autoRefresh2()
      // window.setInterval(() => {
      //   this.getRecentMessages()
      // }, 300000)
      this.hIcon = './img/H.png'
      // this.profile = `http://hrdapps48:3001/ftp/employee_pictures/0${this.userInfo.employeeCode}.jpg`
      // temporary
      if(this.userInfo != 'a'){
    ...
    data: () => ({
      ...
      items: [
        ...
        { icon: 'mdi-account-supervisor', text: 'Master List', to: 'masterlistmaintenance' },
        { text: 'Documentation', icon: 'mdi-file-document-box-multiple-outline', to: 'hrddoc' },
      ...
      task:{ text:'Tasks',icon:'mdi-clipboard-list-outline',to:'task'},
      system:{text:'Systems',icon:'mdi-view-list',to:'systemlist'},
      manage:{text:'System Presentation',icon:'mdi-format-list-checks',to:'presentation'}
    }),
    computed: {
      profile() {
